const RenderFirework = () => {
  ...
  var a = document.getElementById('firework');

  if (a.firstChild === null) {
    let box = document.getElementById('box-result-id');
        ...
        if (rootShow) {
          ...
          if (exParticles.length === 0) {
            ...
            for (let i = 0; i < exPLen; i++) {
              exParticles.push(new particle(this.pos, new vector(-rndNum(8) + 5, -rndNum(8) + 5)));
    ...
    function draw(time) {
      update(time);
      ctx.fillStyle = 'rgba(131, 131, 131, 0.6)';
      if (flash) {
        flash = false;
      }
